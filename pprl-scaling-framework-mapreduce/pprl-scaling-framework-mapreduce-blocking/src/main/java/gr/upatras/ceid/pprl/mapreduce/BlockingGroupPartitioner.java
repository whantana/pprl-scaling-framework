...
public class BlockingGroupPartitioner extends Partitioner<Text, Text> implements Configurable{
    ...
    public int getPartition(Text key, Text value, int numPartitions) {
        final int blockId = Integer.valueOf(key.toString().split("_")[0]); // key example 00000_001100_A
        assert blockId >= 0 && blockId < L;
        if(L == R) return blockId;
        return (new RangesInRanges(L-1,0,R)).belongsToRange(blockId);
    }
