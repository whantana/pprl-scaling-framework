...
public class EncodingCommands implements CommandMarker {
    ...
    public String command1(
            @CliOption(key = {"avgQgrams"}, mandatory = false, help = "(Optional) Override average q-gram estimation for given Q.")
            final String aqStr,
            @CliOption(key = {"weights"}, mandatory = false, help = "(Optional) Override weight estimation .")
            final String wStr,
            @CliOption(key = {"stats"}, mandatory = false, help = "(Optional) Path to property file containing the required data statistics.")
            final String pathStr,
            @CliOption(key = {"fields"}, mandatory = false, help = "(Optional) Field names.")
            final String fStr,
            @CliOption(key = {"N"}, mandatory = false, help = "(Optional) N for static bloom filter length. Default is 256.")
            final String nStr,
        ...
        try {
            ...
            final int N = CommandUtil.retrieveInt(nStr, 256);
            ...
            if(statsPath != null) {
                DatasetStatistics statistics = (statsPath.toString().startsWith("hdfs://")) ?
                        ds.loadStats(statsPath) : lds.loadStats(statsPath);
            LOG.info(DatasetStatistics.prettyBFEStats(statistics.getFieldStatistics(), N, K, Q));
            LOG.info("\n");
            } else {
                String[] fieldNames = CommandUtil.retrieveFields(fStr);
                double[] avgQgrams = CommandUtil.retrieveDoubles(aqStr);
                double[] weights = CommandUtil.retrieveDoubles(wStr);
                LOG.info(DatasetStatistics.prettyBFEStats(fieldNames,avgQgrams,weights,N, K, Q));
                LOG.info("\n");
            }
            return "DONE";
        } catch (Exception e) {
    ...
    public String command2(
        ...
        try {
            ...
            if(statsPath != null) {
                ...
                int i = 0;
                avgQgrams = new double[fields.length];
                weights = new double[fields.length];
            ...
            if(aqStr != null) avgQgrams = CommandUtil.retrieveDoubles(aqStr);
            if(wStr  != null) weights = CommandUtil.retrieveDoubles(wStr);
