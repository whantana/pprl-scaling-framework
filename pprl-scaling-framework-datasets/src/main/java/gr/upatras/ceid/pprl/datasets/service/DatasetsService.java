...
public class DatasetsService {
    ...
    public void importDblpXmlDataset(final File localDatasetFile, final File localSchemaFile, final String datasetName)
        ...
        try {
            ...
            LOG.info("Dataset : " + datasetName + ", Avro Data files Path : " + files + ".");
            runDblpXmlToAvroJob(input, files);
    ...
    public Path makeDatasetDirectory(final String datasetName, final boolean makeDirs)
            throws IOException {
        try {
            final Path path = new Path(hdfs.getHomeDirectory() + "/" + datasetName);
            ...
            hdfs.mkdirs(path, ONLY_OWNER_PERMISSION);
            ...
            if (makeDirs) {
                ...
                hdfs.mkdirs(dataDirectory, ONLY_OWNER_PERMISSION);
                hdfs.mkdirs(schemaDirectory, ONLY_OWNER_PERMISSION);
    ...
    public Path uploadFileToHdfs(final File file, final Path path, final FsPermission permission)
        ...
        try {
            ...
            hdfs.setPermission(destination, permission);
    ...
    public void runDblpXmlToAvroJob(final Path input, final Path output)
            throws Exception {
        try {
            MultiTagXmlInputFormat.setInputPaths(dblpXmlToAvroJob, input);
            AvroKeyOutputFormat.setOutputPath(dblpXmlToAvroJob, output);
            dblpXmlToAvroJobRunner.call();
            hdfs.setPermission(output, new FsPermission(FsAction.ALL, FsAction.NONE, FsAction.NONE, false));
        } catch (InterruptedException e) {
            LOG.error(e.getMessage());
            throw e;
        } catch (IOException e) {
    ...
    public List<String> makeDatasetTable(final String datasetName, final Path avroPath, final Path schemaPath)
		...
		final String properAvroPath = hdfs.isDirectory(avroPath) ? avroPath.toString() : avroPath.getParent().toString();
		
		final Matcher schemaMatcher = HDFS_URL_PATTERN.matcher(schemaPath.toString());
		final Matcher avroMatcher = HDFS_URL_PATTERN.matcher(properAvroPath.toString());
		
		if (!schemaMatcher.matches() || !avroMatcher.matches()) {
			LOG.error("hdfs path shortening regex does not match on : " + schemaPath.toString() +".");
			LOG.error("hdfs path shortening regex does not match on : " + properAvroPath.toString() +".");
			return null;
		}
