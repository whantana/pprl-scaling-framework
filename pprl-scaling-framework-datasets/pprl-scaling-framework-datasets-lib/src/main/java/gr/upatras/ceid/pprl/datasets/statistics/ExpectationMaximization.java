...
public class ExpectationMaximization {
    ...
    public void runAlgorithm(final Gamma gamma){
		pairCount = gamma.getPairCount();
        for(iteration = 1; iteration <= MAX_ITERATIONS; iteration++ ) {
            double [][] g = runExpecationStep(gamma);
            boolean converges = runMaximizationStep(gamma, g);
			if(converges) break;
        }
    }
    ...
    private double[][] runExpecationStep(final Gamma gamma) {
		double[][] g = new double[pairCount][2];
		for(int i = 0; i < pairCount ; i++) {
			double a = p;
			double b = (1 - p);
			for(int j = 0; j < fieldCount ; j++) {
				a *= gamma.get(i,j) ? m[j] : (1 - m[j]);
				b *= gamma.get(i,j) ? u[j] : (1 - u[j]);
			}
			g[i][0] = a/(a+b);
			g[i][1] = b/(a+b);
		}
	...
	private boolean runMaximizationStep(final Gamma gamma, double[][] g) {
		double mSum = 0.0;
		double uSum = 0.0;
		for(int j=0; j < fieldCount; j++) {
			double a = 0.0;
			double b = 0.0;
			for(int i=0;i< pairCount ;i++) {
				a += gamma.get(i,j) ? g[i][0] : 0;
				b += gamma.get(i,j) ? g[i][1] : 0;
				if(j == 0) {
					mSum += g[i][0];
					uSum += g[i][1];
				}
