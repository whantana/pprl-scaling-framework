...
//        };
	...
	public void uploadDataset(final String name) throws IOException{
		Path base = new Path("data");
		final Path localDatasetBase = new Path(base,"dl_" + name);
		final Path[] sourceAvro = 
			new Path[]{new Path(localDatasetBase,"avro")};
		final Path sourceSchema = 
			new Path(localDatasetBase, "schema/" + name + ".avsc");
		ds.uploadFiles(sourceAvro,sourceSchema,name,DatasetsService.OTHERS_CAN_READ_PERMISSION);
	}
